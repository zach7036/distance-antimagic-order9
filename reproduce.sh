...
require_hash() {
  ...
  actual="$(hash_of "$file")"
  [[ "$actual" == "$expected" ]] || { echo "HASH MISMATCH $file" >&2; echo "expected $expected" >&2; echo "actual   $actual" >&2; exit 1; }
  echo "OK $actual  $file"
}
...
then
  ...
  echo 'NetworkX not installed; standard-library verifier completed. Install requirements.txt to run the second verifier.'
fi
...
if [[ "$MODE" == "primary" ]]; then exit 0; fi
if [[ "$MODE" != "full" ]]; then echo 'Usage: ./reproduce.sh [primary|full]' >&2; exit 2; fi
